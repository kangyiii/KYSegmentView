...
@implementation KYBaseSegmentView
{
    UIView *lineBottom;
...
- (void)setTopTabHiddenEnable:(BOOL)topTabHiddenEnable {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.topTab.frame = CGRectMake(0, 0 - minusDistance, WIDTH, _topHeight);
        self.scrollView.frame = CGRectMake(0, _topHeight - minusDistance, WIDTH, self.frame.size.height - (_topHeight - minusDistance));
    }];
}
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        _scrollView.tag = 318;
        _scrollView.backgroundColor = Color(250, 250, 250);
        [self updateScrollViewUI];
...
- (void)touchAction:(UIButton *)button {
    [_scrollView setContentOffset:CGPointMake(WIDTH * button.tag, 0) animated:YES];
    self.currentPage = (WIDTH * button.tag + WIDTH / 2) / WIDTH;
}

#pragma mark - UIScrollViewDelegate
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView.tag == 318) {
        self.currentPage = (NSInteger)((scrollView.contentOffset.x + WIDTH / 2) / WIDTH);
    }
}

- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView.tag == 318) {
        NSInteger yourPage = (NSInteger)((scrollView.contentOffset.x + WIDTH / 2) / WIDTH);
        CGFloat yourCount = 1.0 / _titleArray.count;
        if (_titleArray.count > 5) {
            yourCount = 1.0 / 5.0;
        }
        if (_autoFitTitleLine) {
            _bottomLinePer = [bottomLineWidthArray[yourPage] floatValue] / (WIDTH * yourCount);
        }
        CGFloat lineBottomDis = yourCount * WIDTH * (1 -_bottomLinePer) / 2;
        if (topTabType == 1) {
        ...
        if (_titleArray.count > 5) {
            switch (topTabType) {
                case 0:
                    if (_bottomLineHeight >= 3) {
                        lineBottom.frame = CGRectMake(scrollView.contentOffset.x / 5 + lineBottomDis, _topHeight - 3, yourCount * WIDTH * _bottomLinePer, 3);
                        break;
                    }
                    lineBottom.frame = CGRectMake(scrollView.contentOffset.x / 5 + lineBottomDis, _topHeight - _bottomLineHeight, yourCount * WIDTH * _bottomLinePer, _bottomLineHeight);
                    break;
                case 1:
                    lineBottom.frame = CGRectMake(scrollView.contentOffset.x / 5 + lineBottomDis, (_topHeight - _blockHeight) / 2.0, yourCount * WIDTH * _bottomLinePer, _blockHeight);
                    break;
                default:
                    break;
            }
            ...
            switch (topTabType) {
                case 0:
                    if (_bottomLineHeight >= 3) {
                        lineBottom.frame = CGRectMake(scrollView.contentOffset.x / _titleArray.count + lineBottomDis, _topHeight - 3, yourCount * WIDTH * _bottomLinePer, 3);
                    }else {
                        lineBottom.frame = CGRectMake(scrollView.contentOffset.x / _titleArray.count + lineBottomDis, _topHeight - _bottomLineHeight, yourCount * WIDTH * _bottomLinePer, _bottomLineHeight);
                    }
                    break;
                case 1:
                    lineBottom.frame = CGRectMake(scrollView.contentOffset.x / _titleArray.count + lineBottomDis, (_topHeight - _blockHeight) / 2.0, yourCount * WIDTH * _bottomLinePer, _blockHeight);
                    break;
                default:
                    break;
            }
...
- (void)updateScrollViewUI {
    _scrollView.contentSize = CGSizeMake(WIDTH * _titleArray.count, 0);
    if (!_slideEnabled) {
...
- (void)updateTopTabUI {
    ...
    _topTab.contentSize = CGSizeMake((1 + additionCount) * WIDTH, _topHeight - 40);
    if (_baseDefaultPage > 2 && _baseDefaultPage < _titleArray.count) {
        if (_titleArray.count >= 5) {
            _topTab.contentOffset = CGPointMake(1.0 / 5.0 * WIDTH * (_baseDefaultPage - 2), 0);
        }else {
            _topTab.contentOffset = CGPointMake(1.0 / _titleArray.count * WIDTH * (_baseDefaultPage - 2), 0);
        }
    }
    ...
    for (NSInteger i = 0; i < _titleArray.count; i++) {
        ...
        if (_titleArray.count > 5) {
            ...
            button.frame = CGRectMake(WIDTH / _titleArray.count * i, 0, WIDTH / _titleArray.count, _topHeight);
        }
        if (_topArray.count == _titleArray.count && _changeTopArray.count == _titleArray.count && (topTabType == 0 || topTabType == 2)) {
            UIView *customTopView = _topArray[i];
    ...
    if (!_topTabUnderLineHidden) {
        ...
        topTabBottomLine.backgroundColor = Color(206, 206, 206);
    ...
    if (_underlineBlockColor) {
        ...
        lineBottom.backgroundColor = Color(255, 98, 98);
    }
    ...
    //Create ninaMaskView.
    if (topTabType == 1) {
        ninaMaskView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, (1 + additionCount) * WIDTH, _blockHeight)];
        ...
        for (NSInteger j = 0; j < _titleArray.count; j++) {
            ...
            if (_titleArray.count > 5) {
                ...
                maskLabel.frame = CGRectMake(WIDTH / _titleArray.count * j - WIDTH / _titleArray.count * (1 - _bottomLinePer) / 2, 0, WIDTH / _titleArray.count, _blockHeight);
            }
...
- (void)baseViewLoadData {
    self.topTab.frame = CGRectMake(0, 0, WIDTH, _topHeight);
    self.scrollView.frame = CGRectMake(0, _topHeight, WIDTH, self.frame.size.height - _topHeight);
...
- (void)initUI {
    ...
    if (_autoFitTitleLine) {
        _bottomLinePer = [bottomLineWidthArray[0] floatValue] / (WIDTH * yourCount);
    }
    CGFloat lineBottomDis = yourCount * WIDTH * (1 -_bottomLinePer) / 2;
    ...
    switch (topTabType) {
        case 0:
            if (_bottomLineHeight >= 3) {
                lineBottom.frame = CGRectMake(lineBottomDis, _topHeight - 3, yourCount * WIDTH * _bottomLinePer, 3);
            }else {
                lineBottom.frame = CGRectMake(lineBottomDis + WIDTH * yourCount * defaultPage, _topHeight - _bottomLineHeight, yourCount * WIDTH * _bottomLinePer, _bottomLineHeight);
            }
            break;
        case 1: {
            lineBottom.frame = CGRectMake(lineBottomDis + WIDTH * yourCount * defaultPage, (_topHeight - _blockHeight) / 2.0, yourCount * WIDTH * _bottomLinePer, _blockHeight);
    ...
    if (!_topTabUnderLineHidden) {
        topTabBottomLine.frame = CGRectMake(0, _topHeight - 1, (1 + additionCount) * WIDTH, 1);
    }
}
